...
if ! xor "$E2E_TESTS" "$COMPONENT_TESTS" "$UNIT_TESTS"
then
...
if [ -n "$E2E_TESTS" ]
then
    if ! xor "$TEST_FD2" "$TEST_EXAMPLES" "$TEST_SCHOOL"
    then
    ...
    if ! xor "$DEV_SERVER" "$PREVIEW_SERVER" "$LIVE_FARMOS_SERVER"
    then
...
then
    ...
    if [ "$(check_url localhost:5173/fd2/main/)" == "" ]
    then
...
then
    ...
    if [ "$(check_url localhost:4173/fd2/main/)" == "" ]
    then
